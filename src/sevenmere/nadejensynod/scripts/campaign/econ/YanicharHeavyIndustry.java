...
//Custom industry for Selin. (Hopefully) Acts as Orbital Works with Alpha Core and Pristine Nanoforge installed. All hail the Prophet!
public class YanicharHeavyIndustry extends BaseIndustry {

	public static float ORBITAL_WORKS_QUALITY_BONUS = 0.7f;
	...
	public void apply() {
		...
		supply(Commodities.HEAVY_MACHINERY, size - 2 + mysteryBonus);
		supply(Commodities.SUPPLIES, size - 2 + mysteryBonus);
		supply(Commodities.HAND_WEAPONS, size - 2 + mysteryBonus);
		supply(Commodities.SHIPS, size - 2 + mysteryBonus);
		if (shipBonus > 0) {
			supply(1, Commodities.SHIPS, shipBonus, "Yanichar orbital works");
		}
...
//		}
		...
		if (qualityBonus > 0) {
			market.getStats().getDynamic().getMod(Stats.PRODUCTION_QUALITY_MOD).modifyFlat(getModId(1), qualityBonus, "Yanichar orbital works");
		}
	...
	public boolean isHidden() {
		return !market.getFactionId().equals("nadejensynod");
	}

	@Override
	public boolean isFunctional() {
		return super.isFunctional() && market.getFactionId().equals("nadejensynod");
	}
